...
NWM_soil_params.mult=100.0;    // not from SOILPARM.TBL, This is actually calibration parameter: LKSATFAC
...
for(i=(*start);i<lenny;i++)
  ...
  else if (theString[i]!=' ' && theString[i]!='\t' && theString[i]!=',' && theString[i]!=':' && theString[i]!='/' && theString[i]=='-' && theString[i-1]==',')
    {
      theWord[j]=theString[i];
      j++;
    }
