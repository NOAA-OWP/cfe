...
// following the method in the NWM/CFE parameter equivalence document, assuming field capacity soil
...
cfe->soil_reservoir.coeff_primary = cfe->NWM_soil_params.satdk * cfe->NWM_soil_params.slop * cfe->time_step_seconds;
...
// FLO:  No.  LKSATFAC is allowed to vary between 10 and 10,000.  K_lf is an equivalent calibration factor.
...
// TODO: is this the best way to handle this?  FLO
else
  {
  return amount;
  }
...
{
static double previous_flux_perc_m=0.0;   // static type initialized first call as 0.0 
...
if (model->is_forcing_from_bmi == 1)
  ...
  timestep_rainfall_input_m = model->forcing_data_precip_kg_per_m2[model->current_time_step_count];
...
model->et_struct.potential_et_m_per_timestep = model->et_struct.potential_et_m_per_s * model->time_step_seconds;

if(timestep_rainfall_input_m >0.0)
  {

  // FIXME add mass balance delta.  FLO
  ...
  if(timestep_rainfall_input_m > model->et_struct.potential_et_m_per_timestep)  // take all of rainfall to satisfy PET)
    ...
    //model->et_struct.potential_et_m_per_timestep=0.0; // Do we need this now?  FLO
    } 
  else 
    {
...
double timestep_h = model->time_step_seconds / 3600.0;
...
// FLO static variable previous_flux_per_m initialized as  0.0, and subsequent times it will
// be whatever was set in previous time step flux calculation.

if(model->current_time_step_count > 0) previous_flux_perc_m = *model->flux_perc_m;  // changed from ternary operator FLO

/*########################## This is missing from this code:  FLO
  // check to make sure that there is storage available in soil to hold the water that does not runoff
  //--------------------------------------------------------------------------------------------------
  if(soil_reservoir_storage_deficit_m<infiltration_depth_m)
    {
...
double secondary_flux; // Don't think this is actually used ffor GW reservoir
