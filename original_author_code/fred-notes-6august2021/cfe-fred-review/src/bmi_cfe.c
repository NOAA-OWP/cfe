...
static int Get_end_time (Bmi *self, double * time)
{
    ...
    *time += (((cfe_model *) self->data)->num_timesteps * ((cfe_model *) self->data)->time_step_seconds);
    return BMI_SUCCESS;
}
...
{
    *dt = ((cfe_model *) self->data)->time_step_seconds;
    return BMI_SUCCESS;
}
...
static int Get_current_time (Bmi *self, double * time)
{
    Get_start_time(self, time);
#if CFE_DEGUG > 1
    printf("Current model time step count: '%d'\n", ((cfe_model *) self->data)->current_time_step_count);
#endif
    *time += (((cfe_model *) self->data)->current_time_step_count * ((cfe_model *) self->data)->time_step_seconds);
    return BMI_SUCCESS;
}
...
static int Initialize (Bmi *self, const char *file)
{
    ...
    *******************************************************/
    ...
    #endif
            cfe->forcing_data_precip_kg_per_m2[i] = forcings.precip_kg_per_m2 * ((double)cfe->time_step_seconds);
...
static int Update_until (Bmi *self, double t)
{
    ...
    // Keep in mind the current_time_step_count hasn't been processed yet (hence, using <= for this test)
    // E.g., if (unprocessed) current_time_step_count = 0, t = 2, num_timesteps = 2, this is valid a valid t (run 0, run 1)
    if ((cfe->current_time_step_count + t_int) <= cfe->num_timesteps) {
        for (i = 0; i < t_int; i++)
            run_cfe(cfe);
        return BMI_SUCCESS;
    }
...
static int Get_value_ptr (Bmi *self, const char *name, void **dest)
{
    ...
    if (strcmp (name, "RAIN_RATE") == 0) {
        cfe_model* model = (cfe_model *)(self->data);
        *dest = (void*)(model->forcing_data_precip_kg_per_m2 + model->current_time_step_count - 1);
        return BMI_SUCCESS;
    }

    if (strcmp (name, "SURFACE_PRESSURE") == 0) {
        cfe_model* model = (cfe_model *)(self->data);
        *dest = (void*)(model->forcing_data_surface_pressure_Pa + model->current_time_step_count);
        return BMI_SUCCESS;
    }
...
cfe_model *new_bmi_cfe(void)
{
    ...
    data->time_step_seconds = 3600;                  // FLO note this var is an integer 
